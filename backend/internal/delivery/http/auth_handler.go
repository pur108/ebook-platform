...
func (h *AuthHandler) Login(c *fiber.Ctx) error {
	type Request struct {
		...
		Email      string `json:"email"`      // For backward compatibility
		Password   string `json:"password"`
	}
	...
	// Use Identifier if provided, otherwise fallback to Email
	identifier := req.Identifier
	if identifier == "" {
		identifier = req.Email
	}

	token, user, err := h.authUsecase.Login(identifier, req.Password)
	if err != nil {
